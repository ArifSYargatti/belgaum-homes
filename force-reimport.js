...
const forceReimport = async () => {
  try {
    ...
    // First, delete all on Render
    console.log('🗑️ Clearing Render...');
    const getRes = await fetch('https://belgaum-homes-2.onrender.com/api/properties');
    const renderData = await getRes.json();
    let deleted = 0;
    for (const p of renderData.data) {
    ...
    console.log('🗑️ Deleted', deleted, 'properties from Render');
    
    // Now import fresh
    console.log('📤 Importing fresh...');
    let imported = 0;
